...
export default function ShopDirectContent() {
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-start">
              ...
              <div>
                {specs.map(([key, val]) => (
                  <div key={key} className="spec-row">
                    <span className="text-pit-400 text-xs font-display uppercase tracking-wider w-32 shrink-0">{key}</span>
          ...
          <div className="border border-pit-500 bg-pit-800">
            ...
            <div className="p-6 lg:p-8">
              <h2 className="font-display font-black text-xl text-white uppercase mb-1">Ordina ora</h2>
              <p className="text-data text-xs mb-6">Inserisci i tuoi dati e procedi al pagamento sicuro via Stripe</p>

              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <FormField label="Nome completo" name="name" type="text" value={form.name} onChange={handleChange} placeholder="Mario Rossi" colSpan="col-span-2" />
                  <FormField label="Email" name="email" type="email" value={form.email} onChange={handleChange} placeholder="mario@example.com" colSpan="col-span-2" />
                  <FormField label="Indirizzo" name="address" type="text" value={form.address} onChange={handleChange} placeholder="Via Roma 42" colSpan="col-span-2" />
                  <FormField label="Città" name="city" type="text" value={form.city} onChange={handleChange} placeholder="Milano" />
                  <FormField label="CAP" name="postal" type="text" value={form.postal} onChange={handleChange} placeholder="20100" />
                </div>

                <div>
                  <label className="block text-xs font-display uppercase tracking-wider text-data mb-1.5">Paese</label>
                  <select name="country" value={form.country} onChange={handleChange} className="w-full bg-pit-700 border border-pit-500 text-white text-sm px-3 py-3 focus:outline-none focus:border-amber transition-colors appearance-none">
                ...
                {error && (
                  <div className="bg-red-950/60 border border-red-800 px-4 py-3 text-red-300 text-sm">{error}</div>
                )}
                ...
                <button type="submit" disabled={loading} className="btn-amber w-full text-center disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                  <Lock size={14} />
...
function FormField({ label, name, type, value, onChange, placeholder, colSpan }: {
  label: string; name: string; type: string; value: string
  onChange: (e: React.ChangeEvent<HTMLInputElement>) => void
  placeholder?: string; colSpan?: string
}) {
  return (
    <div className={colSpan}>
      <label className="block text-xs font-display uppercase tracking-wider text-data mb-1.5">{label}</label>
...
function fieldLabel(key: keyof FormData): string {
  const labels: Record<keyof FormData, string> = {
    name: 'Nome completo', email: 'Email', address: 'Indirizzo',
    city: 'Città', postal: 'CAP', country: 'Paese',
  }
  return labels[key]
}
